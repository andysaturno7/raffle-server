...
export const controllers = {
  async setToken(req, res) {
    try {
      ...
      let tk_request = await zoomOAuthApi.setUserApiToken(
  ...
  async getAllParticipants(req, res) {
    try {
      ...
      params["type"] = type ? type : "live";
      do {
        let response = await zoomApiConnection.getMetricsParticipants(
          wm,
          wmId,
          params
        );
      ...
      if (type === "live") data = data.filter((val) => !("leave_time" in val));
      return res.status(200).send(data);
    } catch (error) {
      console.log({ error });
      return res.status(500).send({ mesage: error.message });
